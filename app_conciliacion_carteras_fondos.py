...
def build_reconciliation(dep_pos, bbg_pos, cash_dep=np.nan, cash_bbg=np.nan, tol_titulos=0.0, tol_nominal=1.0, tol_efectivo=1.0):
    ...
    # Si Bloomberg no trae una columna explícita de títulos, no forzamos una diferencia artificial.
    ...
    def estado(row):
        ...
        titulos_ok = True if not compare_titulos else abs(row["Dif_Titulos"]) <= tol_titulos
        nominal_ok = True if not compare_nominal else abs(row["Dif_Nominal"]) <= tol_nominal
        efectivo_ok = True if not compare_efectivo else abs(row["Dif_Efectivo"]) <= tol_efectivo

        if titulos_ok and nominal_ok and efectivo_ok:
            return "OK"
        return "Diferencia"
...
with st.sidebar:
    ...
    tol_titulos = st.number_input("Tolerancia títulos", min_value=0.0, value=0.0, step=1.0)
...
if dep_file and bbg_file:
    try:
        ...
        status_counts = recon["Estado"].value_counts().to_dict()
        m1, m2, m3, m4 = st.columns(4)
        m1.metric("OK", status_counts.get("OK", 0))
        m2.metric("Diferencias", status_counts.get("Diferencia", 0))
        m3.metric("Solo depositario", status_counts.get("Solo Depositario", 0))
        m4.metric("Solo Bloomberg", status_counts.get("Solo Bloomberg", 0))
